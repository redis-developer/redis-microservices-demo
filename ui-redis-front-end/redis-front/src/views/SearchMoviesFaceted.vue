  ...
  <div 
    ...
    <b-card no-body >
      ...
      <b-card-footer class="small">
        ...
        {{ Number.parseFloat(doc.fields.rating).toFixed(1) }}
        </b-col>
        </b-row>
      </b-card-footer>
...
export default {
  ...
  methods : {
    ...
    async loadMovieGenre() {
      const {data} = await RediSearchRepository.getMovieGroupBy("genre");
    ...
    async search() {
      ...
      const {data} = await RediSearchRepository.searchWithPagination(
                          this.searchQuery, 
                          this.searchOffset, 
                          this.searchLimit, 
                          this.sortBy);
